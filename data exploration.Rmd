...
ggpairs(cdi[,-c(1,2,3)])
```
From the pairwise plot, we observed the correlation and distribution between each two variables. Although the high absolute value of correlation indicate strong relatinship, it may not be lienarly relation. The association between crime rate to each variable was displayed in the last row. No strong linear relationship was detected between crime rate per 1000 person and other variables. 
...
```
According to the boxplot, there is no outliers, which means ther is no sates with unusual crime. rate.
```{r}
cdi_desc = cdi %>%
